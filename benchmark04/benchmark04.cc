              ...
              const unsigned int _elblocks)
{
    ...
    std::cout << "nelmt " << nelmt
              << " DOF/s: " << 1.0e-9 * nelmt * (nm0 * nm1) / time_kokkos1
              << "     " << 1.0e-9 * nelmt * (nm0 * nm1) / time_kokkos2
              << "     " << 1.0e-9 * nelmt * (nm0 * nm1) / time_kokkos3
              << "     " << 1.0e-9 * nelmt * (nm0 * nm1) / time_kokkos4
              << "     " << 1.0e-9 * nelmt * (nm0 * nm1) / time_cublas
              << "     " << 1.0e-9 * nelmt * (nm0 * nm1) / time_cuda1 << "     "
              << 1.0e-9 * nelmt * (nm0 * nm1) / time_cuda2 << "     "
              << 1.0e-9 * nelmt * (nm0 * nm1) / time_cuda3 << "     "
              << 1.0e-9 * nelmt * (nm0 * nm1) / time_cuda4 << "     "
              << 1.0e-9 * nelmt * (nm0 * nm1) / time_cuda5 << "     "
              << 1.0e-9 * nelmt * (nm0 * nm1) / time_cuda6 << std::endl;
    std::cout << std::flush;
}
...
int main(int argc, char **argv)
{
    ...
    {
        run_test<double>(size, nq0, nq1, threads, elblocks);
    }
    Kokkos::finalize();
}

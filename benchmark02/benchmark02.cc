...
void run_test(const unsigned int size)
{
  ...
  std::vector<T> result_thrust(1);
  {
    ...
    {
      time.start();
      thrust::transform(ddata1.begin(), ddata1.end(), ddata2.begin(), ddata1.begin(), 
        []__device__(const T& x, const T& y) {
           return x + y;
        });
      time.stop();
      time_thrust = std::min(time_thrust, time.elapsedSeconds());
    }
    result_thrust[0] = thrust::transform_reduce(ddata1.begin(), ddata1.end(), 
  ...
  std::vector<T> result_cuda(1);
  {
    ...
    cudaMemcpy(cuda_vector1, &host_vector1[0], size * sizeof(T), cudaMemcpyHostToDevice);
    cudaMemcpy(cuda_vector2, &host_vector2[0], size * sizeof(T), cudaMemcpyHostToDevice);
    for (unsigned int t = 0; t < n_tests; ++t)
    {
      time.start();
    ...
    result_cuda[0] = thrust::transform_reduce(thrust::device, cuda_vector1, cuda_vector1 + size, 
       []__device__(const T& x){ 
         return x*x; 
       }, 
       (T)0.0, thrust::plus<T>());
  ...
  std::cout << "Size " << size << "           Kokkos      Thrust      Cuda" << std::endl;
  ...
  std::cout << "Size " << size << " GB/s: "
            << sizeof(T) * 3e-9 * size / time_kokkos << " "

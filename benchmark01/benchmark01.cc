...
__global__ void l2norm_vl(T *__restrict__ sums, T *__restrict__ data, unsigned int n)
{
  ...
  if constexpr (vl && std::is_same_v<T, float>)
  {
    ...
    for(unsigned int tid = grid.thread_rank(); tid < n/vecsize; tid += grid.size())
    {
...
template <typename T, bool vl = true>
__global__ void reduce_vl(T *__restrict__ sums, T *__restrict__ data, unsigned int n)
{
  constexpr unsigned int vecsize = (16u / sizeof(T));
  auto grid =  cg::this_grid();
  auto block = cg::this_thread_block();
...
void run_test(const unsigned int size)
{
  ...
  std::vector<T> result_thrust(1);
  {
    ...
    {
      time.start();
      result_thrust[0] = thrust::transform_reduce(ddata.begin(), ddata.end(), 
  ...
  std::vector<T> result_cuda(1);
  {
    ...
    for (unsigned int t = 0; t < n_tests; ++t)
    {
      ...
      cudaMemcpy(result_cuda.data(), result, sizeof(T), cudaMemcpyDeviceToHost);
      time.stop();
  ...
  std::cout << "Size " << size << " GB/s: "
            << sizeof(T) * 1e-9 * size / time_kokkos << " "
